...
def mock_requestget(*args,**kwarg):
    ...
    if args[0] == "https://fr.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&exintro&explaintext&redirects&list=geosearch":
        response = mock_response(data_structure_geo_search)
    elif args[0] == "https://fr.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1":
